...
function matchCommand(argv) {
    ...
    if (two)
        return { name: two.name, rest: argv.slice(2) };
    const one = COMMANDS.find((c) => c.name === argv[0]);
    if (one)
        return { name: one.name, rest: argv.slice(1) };
    return null;
}
/**
 ...
 */
function parseFlags(rest, boolFlags = new Set()) {
    const positional = [];
    const flags = {};
...
function asRole(value, fallback) {
    if (typeof value !== 'string')
        return fallback;
    if (value === 'tenant_admin' || value === 'tenant_user')
        return value;
    ...
    switch (name) {
        case 'login': {
            ...
            if (typeof flags['api-endpoint'] === 'string')
                loginOpts.apiEndpoint = flags['api-endpoint'];
        ...
        case 'whoami': {
            ...
            try {
                ...
                const endpoint = resolveApiEndpoint(typeof flags['api-endpoint'] === 'string' ? flags['api-endpoint'] : undefined);
                writeStderrJson({
                    error: err.message,
        ...
        case 'project new': {
            ...
            if (!slug)
                throw new Error('project new: missing required <slug> argument');
        ...
        case 'project delete': {
            ...
            const slug = positional[0];
            if (!slug)
                throw new Error('project delete: missing required <slug> argument');
        ...
        case 'rollback': {
            ...
            if (!to)
                throw new Error('rollback: --to <sha> is required');
        ...
        case 'grant': {
            ...
            if (!email)
                throw new Error('grant: missing required <email> argument');
        ...
        case 'revoke': {
            ...
            const slugArg = positional[1];
            if (!email)
                throw new Error('revoke: missing required <email> argument');
            const env = asEnvScope(flags.env, 'both');
        ...
        case 'invite': {
            ...
            const email = positional[0];
            if (!email)
                throw new Error('invite: missing required <email> argument');
