...
export function readCredentials() {
    const path = resolveCredentialsPath();
    if (!existsSync(path)) {
        throw new CredentialsError(`Not logged in. No credentials file at ${path}. Run \`btrmnt login\` first.`);
    }
    const mode = statSync(path).mode & 0o777;
    if (mode !== 0o600) {
        throw new CredentialsError(`Refusing to read credentials at ${path}: file mode is 0${mode.toString(8)}, expected 0600. Run \`chmod 600 ${path}\`.`);
    }
    ...
    try {
        ...
        if (err instanceof ConfigError) {
            throw new CredentialsError(`Credentials file at ${path} references an api_endpoint we no longer trust: ` +
