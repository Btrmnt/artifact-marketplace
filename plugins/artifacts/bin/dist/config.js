 ...
 */
export function validateApiEndpoint(candidate, opts = {}) {
    ...
    if (parsed.protocol !== 'https:' && !isLoopback && !allowInsecure) {
        throw new ConfigError(`api endpoint ${JSON.stringify(trimmed)} uses ${parsed.protocol}; ` +
    ...
    if (!isLoopback && !isAllowedHost(host) && !allowUnknownHost) {
        throw new ConfigError(`api endpoint host ${JSON.stringify(host)} is not in the allowlist ` +
            `(${ALLOWED_HOST_SUFFIXES.join(', ')}); ` +
            `set BTRMNT_ALLOW_UNKNOWN_HOST=1 to allow it (do this only if you trust the host).`);
    }
    return trimmed;
}
export function resolveApiEndpoint(flag) {
    ...
    if (pluginOption && pluginOption.trim().length > 0)
        return pluginOption;
 ...
 */
export function credentialsDir(path) {
    return resolve(path, '..');
}
/**
